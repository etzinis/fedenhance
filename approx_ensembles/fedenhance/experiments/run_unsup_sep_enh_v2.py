...
hparams = vars(args)
generators = dataset_setup.enhancement_single_node_setup(hparams)
...
def l1_mixit(rec_sources_wavs, input_active_speakers, input_noises, input_mom):
    ref_mix_1 = normalize_tensor_wav(
        input_active_speakers + input_noises[:, 0:1])
    ref_mix_2 = normalize_tensor_wav(input_noises[:, 1:2])

    er_00 = torch.abs(normalize_tensor_wav(
        rec_sources_wavs[:, 0:1] + rec_sources_wavs[:, 1:2]) - ref_mix_1)
    er_01 = torch.abs(normalize_tensor_wav(rec_sources_wavs[:, 2:3]) - ref_mix_2)

    er_10 = torch.abs(normalize_tensor_wav(
        rec_sources_wavs[:, 0:1] + rec_sources_wavs[:, 2:3]) - ref_mix_1)
    er_11 = torch.abs(normalize_tensor_wav(rec_sources_wavs[:, 1:2]) - ref_mix_2)

    errors = torch.cat([torch.mean(er_00**2 + er_01**2, dim=[-1]),
                        torch.mean(er_10**2 + er_11**2, dim=[-1])], 1)
    return torch.mean(torch.min(errors, -1)[0])
