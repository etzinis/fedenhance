...
for i in range(hparams['n_global_epochs']):
    ...
    for cnt, data in enumerate(train_tqdm_gen):
        ...
        speech_wavs = speech_wavs[torch.randperm(2 * hparams['batch_size'])]
        noise_wavs = noise_wavs[torch.randperm(2 * hparams['batch_size'])]
        ...
        l = back_loss_tr_loss(rec_sources_wavs, input_active_speakers,
        ...
        with torch.no_grad():
            ...
            if hparams['cometml_log_audio']:
                audio_loggers['separation'].log_batch(
                    ...
                    input_mom[0:1],
                    experiment, step=val_step, tag=val_set+'_separation')
                audio_loggers['enhancement'].log_batch(
                    rec_sources_wavs[-1:, :2],
                    input_active_speakers[-1:],
                    input_mom[-1:],
                    experiment, step=val_step, tag=val_set+'_enhancement')
