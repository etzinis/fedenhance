...
def reduce(perm_loss):
    # weighted = perm_loss * src.norm(dim=-1, keepdim=True)
    return perm_loss.mean()

sup_back_loss_tr_loss_name, sup_back_loss_tr_loss = (
    'tr_back_loss_sup_mixit_SISNR',
    asteroid_sdr_lib.PITLossWrapper(asteroid_sdr_lib.PairwiseNegSDR("sisdr"),
                                    pit_from='pw_mtx')
)

unsup_back_loss_tr_loss_name, unsup_back_loss_tr_loss = (
    'tr_back_loss_mixit_SISNR',
...
for i in range(hparams['n_global_epochs']):
    ...
    for cnt, data in enumerate(train_tqdm_gen):
        ...
        source_waveforms = torch.cat([input_active_speakers, input_noises], 1)
        source_waveforms = source_waveforms.cuda()
        mixtures_waveforms = input_active_speakers + input_noises
        mixtures_waveforms = mixtures_waveforms.cuda()
        input_mom = mixtures_waveforms.sum(1, keepdims=True)
        ...
        # est_src_std = rec_sources_wavs.std(-1, keepdim=True)
        # est_src_mean = rec_sources_wavs.mean(-1, keepdim=True)
        # rec_sources_wavs = (rec_sources_wavs - est_src_mean) / (est_src_std + 1e-9)
        ...
        if (torch.rand([1]) < p_supervised).item():
            l = sup_back_loss_tr_loss(rec_sources_wavs, source_waveforms).mean()
        else:
            l = unsup_back_loss_tr_loss(rec_sources_wavs, mixtures_waveforms)
        l.backward()
