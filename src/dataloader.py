...
class NoisyMNIST(Dataset):
    ...
    @staticmethod
    def add_normal(img):
        return random_noise(img, mode='gaussian', mean=0, var=0.05, clip=True)

    @staticmethod
    def add_salt_and_pepper(img):
        return random_noise(img, mode='s&p', salt_vs_pepper=0.5, clip=True)

    @staticmethod
    def mult_normal(img):
        return random_noise(img, mode='speckle', mean=0, var=0.05, clip=True)
...
if __name__ == "__main__":
    from train_config import default
