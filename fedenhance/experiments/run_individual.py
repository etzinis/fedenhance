...
hparams = vars(args)
init_federated_generators_list, val_generators = \
    dataset_setup.enhancement_federated_individual_setup(hparams)

if hparams['selected_node_ids']:
    federated_generators_list = [init_federated_generators_list[idx]
                                 for idx in hparams['selected_node_ids']]
else:
    federated_generators_list = init_federated_generators_list
...
for i, gen_list in enumerate(federated_generators_list):
    local_model = copy.deepcopy(global_model)
    federated_generators_list[i]['local_model'] = local_model
...
def unsup_mixit(rec_sources_wavs, input_active_speakers, input_noises, input_mom):
...
for i in range(hparams['n_global_epochs']):
    ...
    for train_node_id, node_dic in enumerate(training_nodes):
        ...
        node_dic['local_model'] = local_model
    tr_step += 1
